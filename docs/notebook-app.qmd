...
Import PubMed or PubMed Central records directly from a notebook, or stage them
first for screening:
...
- Staging records, files, PubMed, and PubMed Central results as generic
  screening runs before promotion.
...
Direct `from_pubmed(...)` and `from_pmc(...)` imports remain available as
compatibility shortcuts when an intermediate screening step is not needed.
